...
<div class="content poll-result">
    ...
    <div class="wrapper">
       ...
       ->where('ans_id', $ans->ans_id)
       ->select('poll_user_ans.*')
       ->count();
       $pollcount = DB::table('poll_user_ans')
        ->where('poll_id', $ans->poll_id)
       ->select('poll_user_ans.*')
       ->count(); ?>
            <p>{{$ans->ans}} <span>({{$pollresult}} Votes)</span></p>
            <div class="result-bar" style= "width:<?=@round(($pollresult)/$pollcount*100)?>%">
                <?=@round(($pollresult)/$pollcount*100)?>%
            </div>

        </div>
        @endforeach 
<h4>Total Votes:  {{$pollcount}}</h4>
    </div>
